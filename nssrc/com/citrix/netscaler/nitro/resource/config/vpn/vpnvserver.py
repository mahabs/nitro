...
class vpnvserver(base_resource) :
	...
	def servicetype(self) :
		"""Protocol used by the NetScaler Gateway virtual server.<br/>Default value: SSL<br/>Possible values = SSL.
		"""
		try :
	...
	@property
	def ipv46(self) :
		"""IPv4 or IPv6 address of the NetScaler Gateway virtual server. Usually a public IP address. User devices send connection requests to this IP address.<br/>Minimum length =  1.
		"""
		try :
			return self._ipv46
		except Exception as e:
	...
	@property
	def port(self) :
		"""TCP port on which the virtual server listens.<br/>Minimum length =  1<br/>Range 1 - 65535.
		"""
		try :
			return self._port
		except Exception as e:
	...
	def authentication(self) :
		"""Require authentication for users connecting to NetScaler Gateway.<br/>Default value: ON<br/>Possible values = ON, OFF.
		"""
		try :
	...
	@property
	def icaonly(self) :
		"""User can log on in Basic mode only, through either Citrix Receiver or a browser. Users are not allowed to connect by using the NetScaler Gateway Plug-in.<br/>Default value: OFF<br/>Possible values = ON, OFF.
		"""
		try :
	...
	def icaproxysessionmigration(self) :
		"""This option determines if an existing ICA Proxy session is transferred when the user logs on from another device.<br/>Default value: OFF<br/>Possible values = ON, OFF.
		"""
		try :
	...
	def advancedepa(self) :
		"""This option tells whether advanced EPA is enabled on this virtual server.<br/>Default value: OFF<br/>Possible values = ON, OFF.
		"""
		try :
	...
	@property
	def devicecert(self) :
		"""Indicates whether device certificate check as a part of EPA is on or off.<br/>Default value: OFF<br/>Possible values = ON, OFF.
		"""
		try :
	...
	def certkeynames(self) :
		"""Name of the certificate key that was bound to the corresponding SSL virtual server as the Certificate Authority for the device certificate.<br/>Minimum length =  1<br/>Maximum length =  127.
		"""
		try :
	...
	def listenpolicy(self) :
		"""String specifying the listen policy for the NetScaler Gateway virtual server. Can be either a named expression or a default syntax expression. The NetScaler Gateway virtual server processes only the traffic for which the expression evaluates to true.<br/>Default value: "none".
		"""
		try :
	...
	def maxloginattempts(self) :
		"""Maximum number of logon attempts.<br/>Minimum length =  1<br/>Maximum length =  255.
		"""
		try :
	...
	def deploymenttype(self) :
		""".<br/>Default value: 5<br/>Possible values = NONE, ICA_WEBINTERFACE, ICA_STOREFRONT, MOBILITY, WIONNS.
		"""
		try :
	...
	@property
	def type(self) :
		"""The type of virtual server; for example, CONTENT based or ADDRESS based.<br/>Possible values = CONTENT, ADDRESS.
		"""
		try :
			return self._type
		except Exception as e:
	...
	@property
	def curstate(self) :
		"""The current state of the virtual server, as UP, DOWN, BUSY, and so on.<br/>Possible values = UP, DOWN, UNKNOWN, BUSY, OUT OF SERVICE, GOING OUT OF SERVICE, DOWN WHEN GOING OUT OF SERVICE, NS_EMPTY_STR, Unknown, DISABLED.
		"""
		try :
	...
	@property
	def cachetype(self) :
		"""Virtual server cache type. The options are: TRANSPARENT, REVERSE, and FORWARD.<br/>Possible values = TRANSPARENT, REVERSE, FORWARD.
		"""
		try :
	...
	def disableprimaryondown(self) :
		"""Tells whether traffic will continue reaching backup virtual servers even after the primary virtual server comes UP from DOWN state.<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
	...
	def check(cls, client, resource) :
		""" Use this API to check vpnvserver.
		"""
		try :
			if type(resource) is not list :
