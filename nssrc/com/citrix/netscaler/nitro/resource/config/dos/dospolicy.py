...
class dospolicy(base_resource) :
	...
	@property
	def qdepth(self) :
		"""Queue depth. The queue size (the number of outstanding service requests on the system) before DoS protection is activated on the service to which the DoS protection policy is bound.<br/>Minimum length =  21.
		"""
		try :
			return self._qdepth
		except Exception as e:
	...
	def cltdetectrate(self) :
		"""Client detect rate. Integer representing the percentage of traffic to which the HTTP DoS policy is to be applied after the queue depth condition is satisfied.<br/>Maximum length =  100.
		"""
		try :
