...
class vxlan_iptunnel_binding(base_resource) :
	...
	def get(cls, service, id) :
		""" Use this API to fetch vxlan_iptunnel_binding resources.
		"""
		try :
	...
	def count(cls, service, id) :
		""" Use this API to count vxlan_iptunnel_binding resources configued on NetScaler.
		"""
		try :
			obj = vxlan_iptunnel_binding()
			obj.id = id
	...
	def count_filtered(cls, service, id, filter_) :
		""" Use this API to count the filtered set of vxlan_iptunnel_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = vxlan_iptunnel_binding()
