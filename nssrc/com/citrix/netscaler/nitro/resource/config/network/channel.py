...
class channel(base_resource) :
	...
	@property
	def throughput(self) :
		"""Low threshold value for the throughput of the LA channel, in Mbps. In an high availability (HA) configuration, failover is triggered when the LA channel has HA MON enabled and the throughput is below the specified threshold.<br/>Maximum length =  160000.
		"""
		try :
	...
	def bandwidthhigh(self) :
		"""High threshold value for the bandwidth usage of the LA channel, in Mbps. The NetScaler appliance generates an SNMP trap message when the bandwidth usage of the LA channel is greater than or equal to the specified high threshold value.<br/>Maximum length =  160000.
		"""
		try :
	...
	def linkredundancy(self) :
		"""Link Redundancy for Cluster LAG.<br/>Default value: OFF<br/>Possible values = ON, OFF.
		"""
		try :
	...
	def reqthroughput(self) :
		"""Minimum required throughput for an interface. Failover is triggered if the operating throughput of a Link Aggregation (LA) channel for which HAMON is ON falls below this value.<br/>Minimum value =  0<br/>Maximum value =  160000.
		"""
		try :
	...
	@property
	def lldpmode(self) :
		"""Link Layer Discovery Protocol (LLDP) mode for an interface. The resultant LLDP mode of an interface depends on the LLDP mode configured at the global and the interface levels.<br/>Possible values = NONE, TRANSMITTER, RECEIVER, TRANSCEIVER.
		"""
		try :
