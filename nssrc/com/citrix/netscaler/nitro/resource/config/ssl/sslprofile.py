...
class sslprofile(base_resource) :
	...
	def sslprofiletype(self) :
		"""Type of SSL profile.FrontEnd is for front end SSL service or vserver.BackEnd is for backend SSL service.<br/>Default value: FrontEnd<br/>Possible values = BackEnd, FrontEnd.
		"""
		try :
	...
	@property
	def dhcount(self) :
		"""Number of interactions, between the client and the NetScaler appliance, after which the DH private-public pair is regenerated. A value of zero (0) specifies infinite use (no refresh). This parameter is not applicable when configuring a backend profile.<br/>Maximum length =  65534.
		"""
		try :
	...
	@property
	def dh(self) :
		"""State of Diffie-Hellman (DH) key exchange. This parameter is not applicable when configuring a backend profile.<br/>Default value: DISABLED<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
			return self._dh
		except Exception as e:
	...
	@property
	def dhfile(self) :
		"""Name for and, optionally, path to the PEM-format DH parameter file to be installed. /nsconfig/ssl/ is the default path. This parameter is not applicable when configuring a backend profile.<br/>Minimum length =  1.
		"""
		try :
			return self._dhfile
		except Exception as e:
	...
	@property
	def ersacount(self) :
		"""Refresh count for regeneration of RSA public-key and private-key pair. Zero (0) specifies infinite usage (no refresh).This parameter is not applicable when configuring a backend profile.<br/>Maximum length =  65534.
		"""
		try :
	...
	def sesstimeout(self) :
		"""Time, in seconds, for which to keep the session active. Any session resumption request receivedafter the timeout period will require a fresh SSL handshake and establishment of a new SSL session.<br/>Maximum length =  0xFFFFFFFE.
		"""
		try :
