...
class sslocspresponder(base_resource) :
	...
	def cachetimeout(self) :
		"""Timeout for caching the OCSP response. After the timeout, the NetScaler sends a fresh request to the OCSP responder for the certificate status. If a timeout is not specified, the timeout provided in the OCSP response applies.<br/>Default value: 1<br/>Minimum length =  1<br/>Maximum length =  1440.
		"""
		try :
	...
	def batchingdepth(self) :
		"""Number of client certificates to batch together into one OCSP request. Batching avoids overloading the OCSP responder. A value of 1 signifies that each request is queried independently. For a value greater than 1, specify a timeout (batching delay) to avoid inordinately delaying the processing of a single certificate.<br/>Minimum length =  1<br/>Maximum length =  8.
		"""
		try :
	...
	def batchingdelay(self) :
		"""Maximum time, in milliseconds, to wait to accumulate OCSP requests to batch.  Does not apply if the Batching Depth is 1.<br/>Maximum length =  10000.
		"""
		try :
	...
	def resptimeout(self) :
		"""Time, in milliseconds, to wait for an OCSP response. When this time elapses, an error message appears or the transaction is forwarded, depending on the settings on the virtual server. Includes Batching Delay time.<br/>Maximum length =  120000.
		"""
		try :
