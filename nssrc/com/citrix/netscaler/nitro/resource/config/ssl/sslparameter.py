...
class sslparameter(base_resource) :
	...
	def crlmemorysizemb(self) :
		"""Maximum memory size to use for certificate revocation lists (CRLs). This parameter reserves memory for a CRL but sets a limit to the maximum memory that the CRLs loaded on the appliance can consume.<br/>Default value: 256<br/>Minimum length =  10<br/>Maximum length =  1024.
		"""
		try :
	...
	def ocspcachesize(self) :
		"""Size, per packet engine, in megabytes, of the OCSP cache. A maximum of 10% of the packet engine memory can be assigned. Because the maximum allowed packet engine memory is 4GB, the maximum value that can be assigned to the OCSP cache is approximately 410 MB.<br/>Default value: 10<br/>Maximum length =  512.
		"""
		try :
	...
	def cryptodevdisablelimit(self) :
		"""Disabled Crypto Device Limit reboots the system once reached.  A value of zero(0) implies no reboot. .<br/>Default value: 0.
		"""
		try :
