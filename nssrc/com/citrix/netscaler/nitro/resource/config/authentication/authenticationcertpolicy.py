...
class authenticationcertpolicy(base_resource) :
	...
	@property
	def rule(self) :
		"""Name of the NetScaler named rule, or a default syntax expression, that the policy uses to determine whether to attempt to authenticate the user with the authentication server.<br/>Minimum length =  1.
		"""
		try :
			return self._rule
		except Exception as e:
	...
	@property
	def reqaction(self) :
		"""Name of the client cert authentication action to be performed if the policy matches.<br/>Minimum length =  1.
		"""
		try :
