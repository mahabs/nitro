...
class tmsessionaction(base_resource) :
	...
	def sesstimeout(self) :
		"""Session timeout, in minutes. If there is no traffic during the timeout period, the user is disconnected and must reauthenticate to access intranet resources.<br/>Minimum length =  1.
		"""
		try :
	...
	def persistentcookievalidity(self) :
		"""Integer specifying the number of minutes for which the persistent cookie remains valid. Can be set only if the persistent cookie setting is enabled.<br/>Minimum length =  1.
		"""
		try :
